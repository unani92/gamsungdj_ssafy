    ...
    <b-colxx xxs="12">
        <template v-if="isArtist">
          ...
          <b-colxx xxs="12" class="mb-4 pl-0 pr-0" style="display: inline-flex;">
            <div class="pr-3 pl-3 mb-4 glide__slide" v-for="(artist, index) in artists.slice(0,3)" :key="index"  @click="detailArtist(artist.id)">
    ...
    <b-colxx xxs="12">
      <template v-if="isSong">
        ...
        <b-colxx xxs="12" class="mb-4 pl-3 pr-3">
          <b-card class="mb-4" style="text-align: center;">
            <table  class="table table-sm" style="margin-bottom:0px;">
                ...
                <tbody>
                  <tr :class="{'flex-row':true}" v-for="(song, index) in songs.slice(0,songListSize)" v-bind:key="index">
                    ...
                    <td class="mb-0 text-muted text-small w-15 w-sm-100" style="vertical-align: middle;">{{song.artist}}</td>
                    <td class="mb-0 text-muted text-small w-15 w-sm-100" style="vertical-align: middle;">{{song.genre}}</td>
                  ...
                  <!-- <tr :class="{'flex-row':true}" v-for="(song, index) in songs.slice(5,songs.length)" v-bind:key="index" :v-if="moreSong">
                    <td><img :src="song.img" class="list-thumbnail responsive border-0"/></td>
                    <td><p class="list-item-heading mb-0 truncate">{{song.name}}</p></td>
                    <td><p class="mb-0 text-muted text-small w-15 w-sm-100">{{song.artist}}</p></td>
                    <td><p class="mb-0 text-muted text-small w-15 w-sm-100">{{song.genre}}</p></td>
                    <td><div class="glyph-icon simple-icon-control-play"/></td>
                    <td><div class="glyph-icon simple-icon-playlist"/></td>
                    <td><div class="glyph-icon simple-icon-magnifier-add" @click="detailSong(song.id)" style="cursor:pointer;"/></td>
                    <td><div class="glyph-icon simple-icon-heart"/></td>
                  </tr> -->
                </tbody>
            </table>
          </b-card>
